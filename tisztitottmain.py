"""Ez a kód egy szavanna szimulációt valósít meg
   Megtisztítottuk a fölösleges kommentektől
   Típus annotációval láttuk el.
   Külön függvényt hoztunk létre a függvényhívásokra.
"""
...
class Allat:
    ...
    def __init__(
        self, faj: str, pozicio: tuple[int, int], maxeletkor: int, szaporodasiido: int
    ) -> None:
...
class Szimulacio:
    ...
    def allatgeneralo(self, faj: str, pozicio: tuple[int, int]) -> None:
        ...
        ragadozo_eletkor = random.randint(9, 12)
        novenyevo_eletkor = random.randint(11, 14)
        if faj == "novenyevo":
            szaporodasiido = 2
    ...
    def szavanna_frissito(self) -> None:
        ...
        for allat in self.allatok:
            x, y = allat.pozicio
            if allat.faj == "novenyevo":
                self.szavanna[y][x] = "\033[32mN\033[0m"
            else:
                self.szavanna[y][x] = "\033[31mR\033[0m"
    ...
    def ujszulott_allatok_kezelese(self) -> None:
        ...
        self.allatok.update(self.ujszulott_allatok)
        self.ujszulott_allatok = set()
            ...
            for masik_allat in self.allatok:
                if (
                    ...
                    if utod_lehetseges_helye:
                        ...
                        # print(allatpozicio, masik_allat.pozicio, ujutodpozicio)
                        self.allatgeneralo(masik_allat.faj, ujutodpozicio)
                        return True
        return False

    def novenyevo_mozgas(
        self, allatpozicio: tuple[int, int], allatobjektum: Allat
    ) -> None:
    ...
    def fuggvenyhivasok(self):
        ...
        print(f"Szabad cellák száma: {len(self.szabadcellak)}")
        print(f"ragadozók száma: {len(self.ragadozok_cellaja)}")
        print(f"növényevőkcellája: {len(self.novenyevok_cellaja)}")
        for i in self.szavanna:  # Végig iterál a szavannán
            print(*i)  # Kiírja a szavannát soronként
        input("Nyomj entert")  # A következő év szimulációját érjük el az inputtal
        os.system("cls")
        self.jatekev_novelo()
        self.ujszulott_allatok_kezelese()
        self.eletkornovelo()
        self.ehsegnovelo()
        self.meghal()
        self.cellak_frissitese()
        ...
        if allat.eletkor % allat.szaporodasiido == 0:  # Ha szaporodási évebn van
            if not allat.szaporodott_az_evben:  # Ha még nem szaporodott az évben.
                szaporodik = peldany.szaporodas(
                    allat
                )  # Meghívjuk a szaporodás függvént
                ...
                if allat.faj == "novenyevo":
                    peldany.novenyevo_mozgas(allat.pozicio, allat)
                    peldany.cellak_frissitese()
                else:
                    peldany.husevo_mozgas(
                        allat.pozicio, allat
            ...
            if allat.faj == "novenyevo":
                ...
                peldany.husevo_mozgas(allat.pozicio, allat)
                peldany.cellak_frissitese()
        peldany.cellak_frissitese()
